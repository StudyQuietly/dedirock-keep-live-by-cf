...
- 支持同一时间只允许一轮监控任务运行（运行并发锁，为了节省 KV 写入限制默认关闭）。
...
- 支持多渠道通知，可同时推送到通用 Webhook、Server 酱、PushPlus、Telegram Bot、飞书、钉钉和企业微信机器人。
...
在“通知设置”中开启通知后，可新增多个通知渠道；一次监控事件会发送到所有已启用渠道。支持的事件包括 VPS 离线、启动命令成功、恢复在线和检查错误。默认离线通知策略为“达到离线阈值”，避免短暂离线时重复推送。
...
| Server 酱 | SendKey |
| PushPlus | Token，Topic 可选 |
| Telegram Bot | Bot Token，Chat ID |
| 飞书机器人 | Webhook URL，Secret 可选 |
| 钉钉机器人 | Webhook URL，Secret 可选 |
| 企业微信机器人 | Webhook URL |

“启动命令成功”表示 Virtualizor 已接受启动命令；VPS 真实恢复运行由后续检查的“恢复在线”通知确认。通知失败不会中断监控任务，发送结果会记录到事件日志。
...
## Cloudflare KV 写入额度与优化说明

Cloudflare 免费套餐的 KV 写入和删除操作（Class A 操作）限制为 **每日 1000 次**，且由您账户下的所有 Workers 共享。

为了防止频繁的定时器检查（默认每分钟唤醒一次）耗尽每日写入额度，本项目实现了以下优化策略：

1. **禁用运行锁（并发锁）**：由于定时触发任务一般顺序串行执行，默认关闭并发运行锁。关闭后，每日可省去约 2880 次 KV 写入与删除操作。如果有并发重叠风险，可在设置中手动开启 `启用运行锁`。
2. **状态变动触发写入**：如果所有被监控的 VPS 状态稳定，程序**不会**写入 KV。仅在状态改变（如在线 -> 离线，或故障发生/恢复，启动冷却变动）时才会立即写入 KV。
3. **定期同步 (Checkpoint)**：在设置页面中可配置 `状态写入间隔`（默认 `30` 分钟）。在状态稳定无波动时，只有在此时间间隔到达时才会写入一次状态，用于更新面板上的“上次检查时间”。如果设为 `0`，则仅在状态变化时写入。
4. **前端状态插值补全**：由于去除了冗余的每分钟在线数据点，前端采用插值算法自动根据当前状态与变更日志还原 48 格状态历史图。即便数天没有任何状态变化，前端图表依然会呈现连续的绿色在线方块。

依靠以上方案，在日常运行正常时，每日 KV PUT 写入次数可降至 **48 次以下**，能够与其他 Workers 和谐共用 Cloudflare 免费版 KV 空间。
